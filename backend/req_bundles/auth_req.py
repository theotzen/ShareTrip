...
async def refresh_token(cookies: dict = None,
                        base_url: str = base_url,
...
async def logout(cookies: dict = None,
                 base_url: str = base_url,
                 endpoint: str = '/api/auth/logout'):
    try:
        res = await client.get(url=base_url + endpoint, cookies=cookies)
        if res.status_code >= 300:
            raise HTTPException(status_code=res.status_code, detail=res.json()['detail'])
...
async def get_all_users(cookies: dict = None,
                        base_url: str = base_url,
                        endpoint: str = '/api/auth/getAllUsers'):
    try:
        res = await client.get(url=base_url + endpoint, cookies=cookies)
        if res.status_code >= 300:
            raise HTTPException(status_code=res.status_code, detail=res.json()['detail'])
...
async def test_trajet(cookies: dict = None,
                      base_url: str = base_url,
                      endpoint: str = '/api/auth/testTrajet'):
    try:
        res = await client.get(url=base_url + endpoint, cookies=cookies)
        if res.status_code >= 300:
            raise HTTPException(status_code=res.status_code, detail=res.json()['detail'])
...
async def test_get_trajet(trajet_id: str,
                          cookies: dict = None,
                          base_url: str = base_url,
                          endpoint: str = '/api/auth/testGetTrajet/'):
    try:
...
async def test_post_trajet(data: dict,
                           cookies: dict = None,
                           base_url: str = base_url,
                           endpoint: str = '/api/auth/testPostTrajet'):
    try:
        res = await client.post(url=base_url + endpoint, json=jsonable_encoder(data), cookies=cookies)
        if res.status_code >= 300:
            raise HTTPException(status_code=res.status_code, detail=res.json()['detail'])
